# # data_analysis_report_langchain.py

# # --- FORCE non-GUI backend (do this before any matplotlib import) ---
# import os
# os.environ['MPLBACKEND'] = 'Agg'          # Force backend before matplotlib loads

# import matplotlib
# print("MPLBACKEND env:", os.environ.get('MPLBACKEND'))
# import matplotlib.pyplot as plt
# print("matplotlib backend after import:", matplotlib.get_backend())
...
# matplotlib.use("Agg")
# import matplotlib.pyplot as plt
# print("Matplotlib backend (at startup):", matplotlib.get_backend())
...
"""
langchain_agent_with_analysis.py

- Reuses typical analysis functions (optimize_memory, descriptive, missing, outliers, correlations, hypothesis tests, plotting).
- If Ollama is available, creates a LangChain pandas agent for natural language queries.
- Falls back to non-LLM interactive mode otherwise.
"""
...
# ------------------ (Paste your analysis functions here) ------------------
# For brevity I include the key functions you posted earlier. If you already have
# them in a separate file, you can import them instead.
def load_data(path):
...
# (include the rest of your analysis functions exactly as you have them)
# I'll assume the functions defined earlier in your script exist here:
# descriptive_summary, missing_value_report, suggest_imputation,
# duplicate_report, outliers_iqr, outliers_zscore,
# compute_correlations, top_correlated_pairs,
# t_test_between_groups, mannwhitney_between_groups, anova_oneway, chi_square_categorical,
# save_heatmap, plot_histograms, plot_scatter, save_text_report, run_analysis
# (To keep the example short I'll import run_analysis from this module's own implementation.)

# ------------------ Minimal wrapper for run_analysis (reuse your function) ------------------
# If you have the full run_analysis in another file, import it:
# from data_analysis_module import run_analysis
# Otherwise, ensure run_analysis defined below or pasted here (use your version).

# For the agent we need a small function that exposes a few tools:
def quick_summary_tool(df):
    """Return a short text summary (good for LLM outputs)."""
    desc = descriptive_summary(df)
    lines = []
    lines.append(f"Shape: {desc['shape'][0]} rows x {desc['shape'][1]} cols")
    lines.append("Numeric columns: " + ", ".join(desc['num_columns']))
    lines.append("Categorical columns: " + ", ".join(desc['cat_columns']))
    # top 5 missing
    missing = desc['missing_counts']
    if missing:
        lines.append("Top missing (col:count): " + ", ".join([f"{k}:{v}" for k,v in missing.items()][:5]))
    return "\n".join(lines)

# ------------------ LangChain agent setup ------------------
def try_create_langchain_agent(df):
    """
    Attempt to create a LangChain pandas agent using Ollama (local) or OpenAI (cloud).
    Returns agent or None on failure.
    """
    try:
        # Prefer local Ollama if package available
        try:
            from langchain.llms import Ollama
            from langchain.agents import create_pandas_dataframe_agent
            llm = Ollama(model="llama3")
            agent = create_pandas_dataframe_agent(llm, df, verbose=False, handle_parsing_errors=True)
            print("[agent] Created LangChain agent with Ollama local model.")
            return agent
        except Exception as e_local:
            # try cloud OpenAI if user has set OPENAI_API_KEY
            try:
                ...
                if key:
                    llm = ChatOpenAI(temperature=0.0, model="gpt-4o-mini" )  # adjust model as available
                    agent = create_pandas_dataframe_agent(llm, df, verbose=False, handle_parsing_errors=True)
                    print("[agent] Created LangChain agent with OpenAI (cloud).")
                    return agent
                else:
                    print("[agent] OPENAI_API_KEY not set; skipping cloud setup.")
            except Exception as e_cloud:
                print("[agent] Cloud LLM not available:", e_cloud)
            print("[agent] Local and cloud LLM creation failed:", e_local)
            return None
    except Exception as e:
        print("[agent] Exception during agent setup:", e)
        return None

# ------------------ Fallback non-LLM interactive (rule-based) ------------------
def fallback_interactive(df):
    print("\n[INTERACTIVE FALLBACK] No LLM found. Use direct commands.")
    print("Commands: describe, missing, duplicates, outliers <col>, corr <col1> <col2>, hist <col>, run_report, exit")
    while True:
        q = input(">>> ").strip()
        ...
        cmd = parts[0].lower()
        try:
            if cmd in ("describe","summary"):
                print( quick_summary_tool(df) )
            elif cmd == "missing":
                print(missing_value_report(df).to_string())
            elif cmd == "duplicates":
                c = df.duplicated().sum()
                print("Duplicate rows:", c)
            elif cmd == "outliers":
                if len(parts) < 2:
                    ...
                    print(f"{mask.sum()} outliers (IQR) in {col} — lower={low}, upper={up}")
            elif cmd == "corr":
                if len(parts) >= 3:
                    c1, c2 = parts[1], parts[2]
                    print(df[[c1,c2]].corr().to_string())
                else:
                ...
                if len(parts) < 2:
                    print("specify numeric column")
                else:
                    col = parts[1]
                    p = plot_histograms(df, [col], max_plots=1)
                    print("Saved hist in analysis_outputs/")
            elif cmd == "run_report":
                rpt = run_analysis_from_module(df)  # run_analysis adapted below
...
# ------------------ Helper: Run the full analysis (adapted to accept df) ------------------
# We adapt your run_analysis to accept an already-loaded df and return path
def run_analysis_from_module(df):
    # (re-use your run_analysis code but accept df instead of path)
    # For brevity, call your existing run_analysis by writing df to a temp csv and calling the original,
    # OR better: assume you have a run_analysis(path) function; if not, paste your run_analysis here.
    # We'll attempt to detect if run_analysis(path) exists in module globals:
    if 'run_analysis' in globals() and callable(globals()['run_analysis']):
        # if original run_analysis expects a path, save df temporarily
        try:
            tmp = OUTPUT_DIR / "tmp_for_agent.csv"
            df.to_csv(tmp, index=False)
            return globals()['run_analysis'](str(tmp))
        except Exception as e:
            return f"Failed to call existing run_analysis: {e}"
    else:
        return "run_analysis implementation not found in this module."

# ------------------ Main: load data, start agent or fallback ------------------
def main(csv_path):
    df = load_data(csv_path)
    df = optimize_memory(df)

    # create agent if possible
    agent = try_create_langchain_agent(df)

    if agent:
        print("\n[LLM AGENT READY] Ask questions (type 'exit' to quit). Examples:")
        print(" - 'Show me top 5 rows.'")
        print(" - 'Which columns have missing values?'")
        print(" - 'Plot histogram of Income.'")
        print(" - 'Find correlation between Age and Spending_Score.'\n")
        while True:
            q = input(">>> ").strip()
            if q.lower() in ("exit","quit","q"):
                break
            try:
                # agent.run will use the df context and return results/plots as text/filepaths
                result = agent.run(q)
                print("\n", result, "\n")
            except Exception as e:
                print("Agent error:", e)
                # fallback to simple direct tools if agent fails
                print("Falling back to simple summary:")
                print( quick_summary_tool(df) )
    else:
        fallback_interactive(df)

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python langchain_agent_with_analysis.py path/to/data.csv")
        sys.exit(1)
    main(sys.argv[1])
